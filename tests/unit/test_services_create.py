...
def test_create_customer_ok(fake_repo):
    ...
    cust = Customer(
        id=None, name="Ana", cpf=CPF("12345678909"), email=Email("ana@mail.com")
    )
...
def test_create_customer_repeated_cpf(fake_repo):
    svc = CreateCustomerService(fake_repo)
    cust1 = Customer(
        id=None, name="A", cpf=CPF("12345678909"), email=Email("a@mail.com")
    )
    cust2 = Customer(
        id=None, name="B", cpf=CPF("12345678909"), email=Email("b@mail.com")
    )
    svc.execute(cust1)
